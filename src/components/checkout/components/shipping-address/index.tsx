...
const ShippingAddress: React.FC<ShippingAddressProps> = ({ checked, onChange }) => {
  ...
  const checkEmailExists = async (email) => {
    try {
      const response = await axios.get(`${MEDUSA_BACKEND_URL}/store/deleteCustomer?email=${email}`);
      if (response.data.targetCustomers.length > 0 && response.data.targetCustomers[0].has_account) {
        setEmailExists(true);
      } else {
  ...
  return (
    <div>
      {customer && (customer.shipping_addresses?.length || 0) > 0 && (
        ...
        <div className="mb-6 flex flex-col gap-y-4 bg-red-100 p-4">
          <p className="text-small-regular">
            {`Sign in to avail more discounts on our products.`}
          </p>
          <SignInPrompt />
        </div>
      )}
      {!customer && emailExists && (
        <div className="mb-6 flex flex-col gap-y-4 bg-red-100 p-4">
          <p className="text-small-regular">
            {`We already have a registered user with email ${email}. Sign in to avail more discounts on our products.`}
          </p>
          <SignInPrompt />
        </div>
      )}
